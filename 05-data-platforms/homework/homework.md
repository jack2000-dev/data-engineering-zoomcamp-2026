...
- [x] C) df.write.mode("overwrite").option("replaceWhere", "month = '2024-02'").save(path) — overwrite only the target window
...
- [x] C) dbutils.widgets.text("taxi_types", '["yellow","green"]')
...
- [x] C) Use Repair Run and mark ingest_trips as the first task to re-run
...
- [x] C) @dlt.expect_or_fail("valid_dt", "pickup_datetime IS NOT NULL")
...
- [x] B) Databricks UI → Catalog Explorer → select table → Lineage tab
